...
The sample video was raised out of this repo to the pupil-pkgs top level so it
can be shared by both the pupil-detectors and pye3d-detector benchmarks:

    pupil-pkgs/sample_data/eye1.mp4   (640x480 MJPEG, 120fps, ~418k frames, 16GB)

(It used to live at pupil-detectors/sample_data/eye1.mp4.)

Decoding the whole thing is wasteful, so we dump a contiguous subset to a flat
uint8 file (n*H*W bytes) that bench.py mmaps directly via load_frames().

Usage:
    python bench/extract_frames.py --start 0     --count 2000 --out bench/frames_2000.raw
    python bench/extract_frames.py --start 90000 --count 4000 --out bench/frames_blink.raw
"""
import argparse
import os
...
_REPO = os.path.dirname(_THIS)                 # pupil-detectors/
_PKGS_ROOT = os.path.dirname(_REPO)            # pupil-pkgs/
